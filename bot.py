...
def format_new_run_notification(ex: dict, exercise_id: str, splits_count: int) -> str:
    try:
        ...
        if splits_count > 0:
            split_data = supabase.table("polar_km_splits").select("km_number,pace_display,hr_avg,hr_max,power_avg,cadence_avg,ascent_m,descent_m").eq("exercise_id", exercise_id).order("lap_number").limit(5).execute()
            if split_data.data:
                lines.append("\n*First splits:*")
...
def build_training_context(run_limit: int = 10, sleep_days: int = 7) -> str:
    try:
        ...
        if runs.data:
            ...
            if latest:
                splits = supabase.table("polar_km_splits").select("km_number,pace_display,hr_avg,hr_max,power_avg,cadence_avg,ascent_m,descent_m").eq("exercise_id", latest["polar_exercise_id"]).order("lap_number").execute()
                if splits.data:
...
def send_post_run_debrief(exercise_id: str):
    ...
    try:
        ...
        run         = run_resp.data[0]
        splits      = supabase.table("polar_km_splits").select("km_number,pace_display,hr_avg,hr_max,power_avg,cadence_avg,ascent_m,descent_m").eq("exercise_id", exercise_id).order("lap_number").execute().data or []
...
def handle_message(message):
    ...
    if lower == "/splits":
        try:
            ...
            splits  = supabase.table("polar_km_splits").select("km_number,pace_display,hr_avg,hr_max,power_avg,cadence_avg,ascent_m,descent_m").eq("exercise_id", ex["polar_exercise_id"]).order("lap_number").execute()
    ...
    if lower.startswith("/resync") and lower != "/resyncall":
        try:
            ...
            if split_rows:
                ...
                splits = supabase.table("polar_km_splits").select("km_number,pace_display,hr_avg,hr_max,power_avg,cadence_avg,ascent_m,descent_m").eq("exercise_id", ex_id).order("lap_number").execute()
